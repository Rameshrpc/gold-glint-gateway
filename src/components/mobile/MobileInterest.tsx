...
import { Banknote, Calendar, AlertTriangle, Clock, CheckCircle } from 'lucide-react';
  ...
  const fetchLoans = useCallback(async () => {
    ...
    try {
      ...
      const processedLoans = (data || []).map(loan => {
        ...
        const interestDue = (loan.principal_amount * loan.interest_rate / 100 / 365) * daysSincePayment;
        ...
        {selectedLoan && (
          <div className="p-4 space-y-4">
            ...
            <MobileFormField
              ...
              defaultValue={selectedLoan.interestDue.toString()}
              icon={<Banknote className="w-4 h-4" />}
            />

            <button
              onClick={() => {
                toast.success('For full collection, please use desktop version');
                setSelectedLoan(null);
                navigate(`/interest?loan=${selectedLoan.loan_number}`);
              }}
              className="w-full py-4 rounded-xl gradient-gold text-white font-semibold tap-scale shadow-mobile-md"
            >
              Proceed to Collect
            </button>
          </div>
